...
exports.sendNotification = functions.https.onRequest(async (request, response) => {
  ...
  switch (notificationType) {
    ...
    case "sosRequestTimedOut":
      ...
      break;
    case "sosRequestCanceled":
      notificationTitle = notificationsLang === "en" ? "SOS request canceled" : "تم إلغاء طلب الأستغاثة";
      notificationBody = notificationsLang === "en" ? `Your SOS request sent to ${hospitalName} was canceled` : `تم إلغاء طلب الأستغاثة الذي أرسلته إلى ${hospitalName}`;
      break;
